...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check or execute one OpenAI-compatible image generation request."
    )
...
def read_extra(path: Optional[Path]) -> Dict[str, Any]:
    ...
    protected = {"prompt", "model", "n", "size", "quality", "background", "output_format"}
...
def resolve_mode(requested: str, references: Tuple[Path, ...]) -> str:
...
def build_payload(args: argparse.Namespace, config: Config, prompt: str) -> Dict[str, Any]:
    if args.n < 1:
        raise ImageGenError("--n must be at least 1")
    payload: Dict[str, Any] = {"prompt": prompt, "n": args.n}
...
def request_json(
    url: str, payload: Mapping[str, Any], references: Tuple[Path, ...], mode: str
) -> Dict[str, Any]:
    return {
        "endpoint": url,
        "mode": mode,
...
def run() -> int:
    ...
    try:
        response = post(url, payload, references, mode, config)
        ...
        for index, item in enumerate(items, 1):
            ...
            suffix = detect_image(data, content_type, args.output_format)
